...
//
// Rules:
//  - Only replaces the FIRST occurrence of each term in the document
      ...
      const output = segments.map((seg, i) => {
        if (i % 2 === 1) {
          ...
          if (!isClosing) {
            ...
            if (tag === "span" && depth.glossTerm > 0) depth.glossTerm--;
        ...
        // Text segment — skip if already replaced, or inside protected zone
        if (replaced) return seg;
        if (depth.glossTerm > 0 || depth.glossary > 0 || depth.heading > 0) return seg;
        if (!termRegex.test(seg)) return seg;
  
        // Do the replacement — first match only
        replaced = true;
